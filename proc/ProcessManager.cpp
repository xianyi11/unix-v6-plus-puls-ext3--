...
//	PageTable* m_UserPageTableArray = NULL;
//	PageTable* pgTable = m_UserPageTableArray;
//	u.u_MemoryDescriptor.Initialize();
	/* 父进程的相对地址映照表拷贝给子进程，共两张页表的大小 */
//	Diagnose::Write("pgTable is NULL ? %d\n",pgTable==NULL?1:0);
//	if ( NULL != pgTable )
//	{
////		u.u_MemoryDescriptor.Initialize();
//		Utility::MemCopy((unsigned long)pgTable, (unsigned long)m_UserPageTableArray, sizeof(PageTable) * MemoryDescriptor::USER_SPACE_PAGE_TABLE_CNT);
//	}
	...
	{
		int n = current->p_size / 4096;
		child->p_addr = desAddress;
		while (n--)
		{
			Utility::CopySeg2(srcAddress, desAddress);
			srcAddress += PageManager::PAGE_SIZE;
			desAddress += PageManager::PAGE_SIZE;
		}
	}
	u.u_procp = current;
	/* 
...
void ProcessManager::Exec()
{
	...
	Diagnose::Write("Process %d execing u.u_arg[0] = %d,u.u_arg[1] = %d,u.u_arg[2] = %d\n",u.u_procp->p_pid,u.u_arg[0],u.u_arg[1],u.u_arg[2]);
